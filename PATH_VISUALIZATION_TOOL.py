...
class Spot:
    ...
    def draw(self, win):    # Draw the spot on window
        pygame.draw.rect(win, self.color, (self.x, self.y, self.width, self.width))

    def update_neighbors(self, grid): # Update neighbor list
...
def algorithm( draw , grid , START , END):
    ...
    while not open_set.empty():
        ...
        if current != START:
            current.make_closed()
    return False # Means path not find                
...
def main( win, width):
    ...
    while run:
        ...
        for i in range(ROWS):   # Draw the border
            spot1 = grid [0][i]
            spot2 = grid[i][0]
            spot3 = grid [49][i]
            spot4 = grid[i][49]
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:    
                if event.key == pygame.K_SPACE and START and END: # Space is pressed
                    for row in grid:
                        for spot in row:
                            spot.update_neighbors(grid)

                    # Checking if path not found or not 
                    if not algorithm( lambda: draw( win, grid, ROWS, width), grid,  START, END): # If not find then run
                        # Creating a window
                        root = Tk()
                        ...
                        # Printing the text 
                        myLabel = Label( root , text = "Path not found !!" , pady = 50)
                        myLabel.pack()
